import 'dart:io';
import 'dart:convert';
import 'package:budget_info/models/budget_settings.dart';
...
class ExportImportFiles {
  static String _prefsSettingsFilename = 'budget_info_settings.json';
  static String _prefsSettingsBackupFilename = 'budget_info_settings.json.bak';
  ...
  static Future<String> getPrivateDirectoryPath() async {
    ...
    final budgetInfoPath = p.join(directory.path, 'BudgetInfo');
    final budgetInfoDir = Directory(budgetInfoPath);
    if (!await budgetInfoDir.exists()) await budgetInfoDir.create(recursive: true);
    return budgetInfoPath;
  }
  ...
  Future<void> savePrefsData(BuildContext context, SharedPreferences prefs, bool silent) async {
    ...
    final budgetInfoPath = await getPrivateDirectoryPath();
    final targetFilePathName = p.join(budgetInfoPath, _prefsSettingsFilename);
    final targetFile = File(targetFilePathName); 
    final backupFilePathName = p.join(budgetInfoPath, _prefsSettingsBackupFilename);
  ...
  Future<bool> importPrefsData(BuildContext context, SharedPreferences prefs) async {
    try {
      // Get or create target directory
      final budgetInfoPath = await getPrivateDirectoryPath();
      final sourceFilePathName = p.join(budgetInfoPath, _prefsSettingsFilename);
  ...
  Future<void> exportAllFilesAsZip(BuildContext context, BudgetSettings settings) async {
    try {
      // Get or create target directory
      final budgetInfoPath = await getPrivateDirectoryPath();
      final budgetInfoDir = Directory(budgetInfoPath);
      ...
      // Get all files in the budgetInfo directory
      final files = budgetInfoDir.listSync();
      if (files.isEmpty) {
      ...
      final datedZipFilePathName = p.join(tempDir.path, DatedFileName('budget_info_backup.zip'));
  ...
  Future<bool> importAllFilesFromZip(BuildContext context) async {
    try {
      ...
      for (final file in archive) {
        ...
        final outputFilePathName = p.join(budgetInfoDir.path, file.name);
